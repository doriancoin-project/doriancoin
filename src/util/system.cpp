...
const char * const BITCOIN_CONF_FILENAME = "doriancoin.conf";
...
#else
    const char* pszModule = "doriancoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Doriancoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Doriancoin";
#else
    // Unix-like
    return pathRet / ".doriancoin";
#endif
#endif
}
