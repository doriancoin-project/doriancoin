...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // Doriancoin: Only support BIP125 RBF when -mempoolreplacement arg is set
    ...
    bool taproot_active = (tip != nullptr) && (tip->nHeight + 1 >= params.TaprootHeight);
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    // Start enforcing Taproot (BIPs 340-342)
