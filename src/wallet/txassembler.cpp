    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional DSV outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes DSV and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::DSV_TO_DSV) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_dsv);
        }
    } else {
        // First try to construct a DSV-to-DSV transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::DSV_ONLY;
        ...
        // If DSV-to-DSV fails, create a peg-out transaction
